...
function search() {
    ...
    const doRelate = (eng, val) => {
        ...
        const local = Base.localRelateSearch(val);
...
function createBlock(index, info, links) {
    ...
    $action_collapse.addEventListener("click", () => {
        ...
        Net.updateCollapse({ type: "one", index: index, status: shown ? 0 : 1 });
    });
...
function linkBlocks() {
    ...
    $action_blocks_collapse.addEventListener("click", () => {
        ...
        if ($expanded_list.length > 0) {
            ...
            Net.updateCollapse({ type: "all", status: 0 });
        } else {
            ...
            Net.updateCollapse({ type: "all", status: 1 });
        }
    })
    Net.detail().then(res => {
        if (res.code !== 200) {
